...
import { fetchInstructorDoubts, answerInstructorDoubt } from "../../redux/doubtSlice";
import { getSocket } from "../../socket";
import toast from "react-hot-toast";
import { Send, Clock, User, MessageCircle, FileText, ImageIcon } from "lucide-react";
...
const InstructorDoubts = () => {
  ...
  const { instructorDoubts, instructorStatus } = useSelector((state) => state.doubts);
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <h1 className="text-2xl font-bold mb-6 text-gray-800">My Assigned Doubts</h1>
      ...
      <div className="space-y-8 mb-12">
        {pendingDoubts.map((d) => (
          <div key={d._id} className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden flex flex-col md:flex-row">
            ...
            <div className="md:w-3/5 p-6 border-b md:border-b-0 md:border-r border-gray-100">
               ...
               <div className="flex justify-between items-start mb-4">
                  ...
                  <span className="text-xs text-gray-500 flex items-center gap-1 font-medium bg-gray-50 px-2 py-1 rounded-full">
                     <User size={12} /> {d.student?.firstname} {d.student?.lastname}
                  </span>
               </div>
            ...
            <div className="md:w-2/5 p-6 bg-gray-50 flex flex-col">
               <label className="text-sm font-bold text-gray-700 mb-2">Your Explanation</label>
               <textarea
                 ...
                 value={answers[d._id] || ""}
                 onChange={(e) => setAnswers(p => ({ ...p, [d._id]: e.target.value }))}
               />
               <button 
        ...
        {pendingDoubts.length === 0 && (
            <div className="text-center py-10 bg-gray-50 rounded-lg border border-dashed border-gray-300">
                <p className="text-gray-400">Great job! You have no pending doubts.</p>
            </div>
        )}
      </div>

      {/* ANSWERED SECTION */}
      <h3 className="text-lg font-bold text-gray-700 mb-4 border-t pt-8">Answered History</h3>
      <div className="space-y-4 opacity-80">
         {answeredDoubts.map(d => (
            <div key={d._id} className="bg-white p-4 rounded-lg border border-gray-200 shadow-sm flex flex-col md:flex-row justify-between gap-4">
               <div>
                  <p className="text-sm font-bold text-gray-800 mb-1">Q: {d.text}</p>
