...
  Menu, X, BookOpen, Award, MessageCircle, Send, Loader2 
} from 'lucide-react';
...
  // Redux State: Exam Data
  const { reviewData, reviewStatus, reviewError } = useSelector((state) => state.students);
  
  // Local State: Navigation
  const [currentIndex, setCurrentIndex] = useState(0);
  const [isSidebarOpen, setSidebarOpen] = useState(false);

  // Local State: Doubt Modal
...
      const userAnsObj = reviewData.answers?.find(a => a.questionId === (q._id || q.id));
      
      // Determine Status
...
    // 1. Safe access to Mock Test ID from reviewData
    const mockTestId = reviewData?.mockTest?._id || reviewData?.mocktestId?._id || reviewData?.mocktestId || null;
...
      
      // Ensure subject is not null/empty (Backend requires it)
      subject: currentQ.subject || currentQ.category || "General", 
      
      // Match Mongoose Enum (assuming "mocktest" is the correct enum value)
      type: "mocktest" 
    };
...
        
        // Clear and close after a brief delay for UX
        setTimeout(() => {
...
      setSubmissionStatus("idle");
    }
  }, [doubtText, currentQ, reviewData, attemptId, dispatch]);

  // --- RENDER HELPERS ---
  const getStatusColor = (status) => {
    switch (status) {
      case "correct": return "bg-emerald-100 text-emerald-700 border-emerald-300";
      case "wrong": return "bg-rose-100 text-rose-700 border-rose-300";
...
        <p className="mb-6 text-gray-500">{reviewError || "Unknown error occurred"}</p>
        <button onClick={() => navigate("/student-dashboard")} className="px-6 py-2 bg-gray-800 text-white rounded-lg">
          Go to Dashboard
        </button>
      </div>
    );
...
      
      {/* --- SIDEBAR (NAVIGATION) --- */}
      <aside className={`fixed inset-y-0 left-0 z-40 w-72 bg-white/80 backdrop-blur-md border-r border-gray-200 transform transition-transform duration-300 lg:relative lg:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
...
                <div className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-rose-500"></span> Wrong</div>
                <div className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-gray-400"></span> Skip</div>
             </div>
          </div>
...
                 // Determine color
                 let colorClass = "bg-gray-100 text-gray-400 border-transparent hover:bg-gray-200";
                 if (q.status === "correct") colorClass = "bg-emerald-500 text-white shadow-md shadow-emerald-200";
                 if (q.status === "wrong") colorClass = "bg-rose-500 text-white shadow-md shadow-rose-200";
                 
...
             <button onClick={() => navigate("/student-dashboard")} className="w-full py-3 bg-gray-900 text-white rounded-xl font-bold text-sm shadow-lg hover:bg-black transition">
                Exit Review
             </button>
          </div>
        </div>
...
           </button>
           <span className="font-bold text-gray-800">Q {currentIndex + 1} / {processedQuestions.length}</span>
           <button onClick={() => navigate("/student-dashboard")} className="p-2 bg-gray-100 rounded-lg">
...
              
              {/* STATUS BANNER */}
              <div className={`p-4 rounded-xl border flex flex-wrap items-center justify-between shadow-sm gap-3 ${getStatusColor(currentQ?.status)}`}>
                 <div className="flex items-center gap-3">
                    {currentQ?.status === "correct" && <CheckCircle className="w-6 h-6" />}
                    {currentQ?.status === "wrong" && <XCircle className="w-6 h-6" />}
                    {currentQ?.status === "unanswered" && <AlertCircle className="w-6 h-6" />}
                    <span className="text-lg font-bold capitalize">{currentQ?.status} Answer</span>
                 </div>
                 <span className="text-sm font-semibold opacity-75">Marks: {currentQ?.status === 'correct' ? `+${currentQ.marks}` : currentQ?.status === 'wrong' ? `-${currentQ.negative}` : '0'}</span>
              </div>

              {/* PARENT PASSAGE */}
              {currentQ?.parentQuestionId && (
                 <div className="bg-blue-50 border-l-4 border-blue-500 p-6 rounded-r-xl shadow-inner">
                    <h3 className="text-blue-900 font-bold mb-2 flex items-center gap-2"><BookOpen size={18} /> Reference Passage</h3>
                    {/* ⭐ FIX: Use questionText from parentQuestionId */}
                    <p className="text-gray-700 whitespace-pre-line text-sm leading-relaxed">{currentQ.parentQuestionId.questionText}</p>
                    {currentQ.parentQuestionId.questionImageUrl && (
                      <img src={getImageUrl(currentQ.parentQuestionId.questionImageUrl)} className="mt-4 rounded-lg border w-full max-h-60 object-contain bg-white" alt="Passage" />
                    )}
                 </div>
              )}
...
                           <span className="text-cyan-500 mr-2">Q{currentIndex + 1}.</span>
...
                           <img src={getImageUrl(currentQ.questionImageUrl)} className="w-full max-h-96 object-contain" alt="Question" />
                        </div>
                    )}

                    {/* OPTIONS */}
                    <div className="space-y-3 mt-6">
                       {currentQ?.questionType === "mcq" && currentQ.options.map((opt, idx) => {
                          const isUserSelected = String(idx) === String(currentQ.userSelected); 
                          const isCorrectOption = currentQ.correct?.includes(idx);

                          // Style Logic
                          let cardStyle = "border-gray-200 bg-white hover:bg-gray-50";
                          let icon = <span className="w-6 h-6 rounded-full bg-gray-100 text-gray-500 flex items-center justify-center text-xs font-bold">{String.fromCharCode(65 + idx)}</span>;

                          if (isCorrectOption) {
                             cardStyle = "border-emerald-500 bg-emerald-50 ring-1 ring-emerald-500";
                             icon = <CheckCircle className="w-6 h-6 text-emerald-600" />;
                          } else if (isUserSelected && !isCorrectOption) {
                             cardStyle = "border-rose-500 bg-rose-50 ring-1 ring-rose-500";
                             icon = <XCircle className="w-6 h-6 text-rose-600" />;
                          }

                          return (
                             <div key={idx} className={`relative p-4 rounded-xl border-2 transition-all flex items-start gap-4 ${cardStyle}`}>
                                <div className="flex-shrink-0 mt-0.5">{icon}</div>
                                <div className="flex-1">
                                   <p className={`text-base font-medium ${isCorrectOption ? 'text-emerald-900' : 'text-gray-700'}`}>{opt.text}</p>
                                   {opt.imageUrl && <img src={getImageUrl(opt.imageUrl)} className="mt-2 h-20 rounded border bg-white" alt="option" />}
                                </div>
                                {isUserSelected && <span className="absolute top-2 right-2 text-[10px] font-bold uppercase tracking-wider px-2 py-1 rounded bg-gray-800 text-white opacity-20">Your Choice</span>}
                             </div>
                          )
                       })}
                       
...
                               <span className="text-xs font-bold text-gray-500 uppercase">Your Answer</span>
                               <p className="text-gray-800 font-medium mt-1">{currentQ.userSelected || "No Answer"}</p>
                            </div>
                            <div className="p-4 bg-emerald-50 rounded-xl border border-emerald-200">
                               <span className="text-xs font-bold text-emerald-600 uppercase">Correct Answer</span>
                               <p className="text-emerald-900 font-medium mt-1">{currentQ.correctManualAnswer}</p>
                            </div>
                         </div>
                       )}
                    </div>
...
                       ) : (
                         <p className="italic text-slate-400">No explanation provided for this question.</p>
                       )}
                    </div>
                 </div>
              </div>
...
                    Raise a Doubt
                 </h3>
                 <button onClick={() => setDoubtModalOpen(false)} className="p-1 hover:bg-gray-200 rounded-full transition">
...
              <div className="p-6">
                 <div className="mb-4">
                    <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Ref Question</label>
                    <p className="text-sm text-gray-600 bg-gray-50 p-3 rounded-lg border border-gray-200 line-clamp-2">
                       {currentQ?.questionText}
                    </p>
                 </div>

                 <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Your Query</label>
                 <textarea 
                    value={doubtText}
...
                       <CheckCircle size={16} /> Doubt submitted successfully!
                    </div>
                 )}
              </div>
